...
Dmitry Hrabrov    1.07 - initial SDL support
*/
...
static void tsReleaseContext(tsContext* ctx)
{
	...
	SDL_CloseAudioDevice(ctx->sdlAudioDevice);
	free(ctx);
}
...
/*void tsAudioCallback(void *userdata, Uint8 *stream, int len) {
tsContext* ctx = (tsContext*)userdata;
*if (audio_len ==0)
return;*
...
SDL_MixAudio(stream, *audio_pos* ctx->samples, len, SDL_MIX_MAXVOLUME);* mix from one buffer into another *
...
tsContext* tsMakeContext(void* unused, unsigned play_frequency_in_Hz, int latency_factor_in_Hz, int num_buffered_seconds, int playing_pool_count)
{
	...
	wanted.samples = sample_count;  /* Good low-latency value for callback */
									/*wanted.callback = tsAudioCallback;*/
									/*wanted.userdata = ctx;*/
	ctx->sdlAudioDevice = 1;
	ret = SDL_OpenAudio(&wanted, NULL);
	TS_CHECK(ret >= 0, "Can't open SDL audio");
	SDL_PauseAudio(0);
	/*ctx->sdlAudioDevice = SDL_OpenAudioDevice(NULL, 0, &wanted, NULL, 0);
	TS_CHECK( ctx->sdlAudioDevice != 0, "Can't open SDL audio device" );
	SDL_PauseAudioDevice(ctx->sdlAudioDevice, 0);*/
...
static void tsPushBytes(tsContext* ctx, void* data, int size)
{
	...
	for (i = 0; i<size; i++)
		if (d[i] != 0) allZero = 0;
	/*printf("Push %d bytes, allzero = %d:\n", size, allZero);*/

	if (!allZero) SDL_QueueAudio(ctx->sdlAudioDevice, data, size);
	/*while (SDL_GetQueuedAudioSize(ctx->sdlAudioDevice) > size/2) {
	SDL_Delay(10);
	}*/
}
#else
...
void tsMix(tsContext* ctx)
{
	...
	/*int samples_to_write = ctx->sample_count;
	if ( !samples_to_write ) goto unlock;
	int bytes_to_write = samples_to_write * ctx->bps;*/
	int queued_bytes = SDL_GetQueuedAudioSize(ctx->sdlAudioDevice);
	int queued_samples = queued_bytes / ctx->bps;
	int latency_samples = ctx->latency_samples;
	int samples_to_write = 0;
	if (queued_samples < 5 * latency_samples) samples_to_write = latency_samples;
	if (!samples_to_write) goto unlock;
	int bytes_to_write = samples_to_write * ctx->bps;
#else
#endif
